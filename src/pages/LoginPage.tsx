  ...
  useEffect(() => {
    if (isAuthenticated()) {
      getUser()
        .then((user) => {
          if (!user.mfa_enabled) {
            navigate('/mfa', { replace: true, state: { mode: 'setup', returnTo } });
          } else {
        ...
        .catch((err) => {
          if (isMfaRequired(err)) {
            navigate('/mfa', { replace: true, state: { mode: 'verify', returnTo } });
          }
        });
    }
  }, [navigate, returnTo]);
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authRes.mfa_required) {
        navigate('/mfa', { state: { mode: 'verify', returnTo } });
        return;
      }
      ...
      if (hasOnlyProfileRead(authRes.abilities) || !user.mfa_enabled) {
        navigate('/mfa', { state: { mode: 'setup', returnTo } });
        return;
      }
        ...
        {activeTab === 'login' ? (
          ...
          <form className={styles.form} onSubmit={handleRegisterSubmit}>
            <div className={styles.userTypeSelector}>
              <button
                type="button"
                className={`${styles.userTypeBtn} ${userType === 'patient' ? styles.userTypeBtnActive : ''}`}
              ...
              <button
                type="button"
                className={`${styles.userTypeBtn} ${userType === 'professional' ? styles.userTypeBtnActive : ''}`}
